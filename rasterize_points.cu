...
// 		out_color.contiguous().data<float>(),
  ...
  return std::make_tuple(rendered, out_semantic, out_depth, out_alpha, radii, geomBuffer, binningBuffer, imgBuffer);
}
...
{
    const int P = means3D.size(0);
    const int H = dL_dout_semantic.size(1);
    const int W = dL_dout_semantic.size(2);
...
//     torch::Tensor dL_dcolors = torch::zeros({P, NUM_CHANNELS}, means3D.options());
    torch::Tensor dL_dsemantic = torch::zeros({P, SEM_CHANNELS}, means3D.options());  //
...
//       dL_dsh.contiguous().data<float>(),
    ...
    return std::make_tuple(dL_dmeans2D, dL_dsemantic, dL_dopacity, dL_dmeans3D, dL_dcov3D, dL_dscales, dL_drotations);
}
