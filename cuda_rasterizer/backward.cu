	...
	float* __restrict__ dL_ddepths)
{
	...
	__shared__ float collected_semantic[SEM_CHANNELS * BLOCK_SIZE];
...
// 	const float T_final = inside ? final_Ts[pix_id] : 0;
	...
	float accum_rec[C] = { 0 };
	float dL_dpixel[C];
	float accum_rec_semantic[SEM_CHANNELS] = { 0 };
	float dL_dpixel_semantic[SEM_CHANNELS];
...
// 			dL_dpixel[i] = dL_dpixels[i * H * W + pix_id];
	...
	float last_semantic[SEM_CHANNELS] = { 0 };  //
...
// 				collected_colors[i * BLOCK_SIZE + block.thread_rank()] = colors[coll_id * C + i];
			for (int i = 0; i < SEM_CHANNELS; i++)
				collected_semantic[i * BLOCK_SIZE + block.thread_rank()] = semantic[coll_id * SEM_CHANNELS + i];
...
// 			}
			...
			// last_alpha = alpha;
			// Update the gradients w.r.t. color of the Gaussian. 
			for (int ch = 0; ch < SEM_CHANNELS; ch++)
			{
				const float s = collected_semantic[ch * BLOCK_SIZE + j];
				...
				dL_dopa += (s - accum_rec_semantic[ch]) * dL_dchannel_semantic;
				atomicAdd(&(dL_dsemantic[global_id * SEM_CHANNELS + ch]), dchannel_dsemantic * dL_dchannel_semantic);
			}

			const float c_d = collected_depths[j];
			accum_rec_depth = last_alpha * last_depth + (1.f - last_alpha) * accum_rec_depth;
			last_depth = c_d;
			dL_dopa += (c_d - accum_rec_depth) * dL_dpixel_depth;
			...
			dL_dopa += (1 - accum_rec_alpha) * dL_dalpha; //- (alpha - accum_alpha_rec) * dL_dalpha;
